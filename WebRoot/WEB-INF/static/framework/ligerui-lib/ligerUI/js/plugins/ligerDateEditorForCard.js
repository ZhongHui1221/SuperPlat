        ...
        updateSelectBoxPosition: function ()
        {
            ...
            if (p.absolute)
            {
            			...
            			&& contentHeight > Number(g.dateeditor.height() + 1))
                {
                    ...
                    g.dateeditor.css({ left: g.text.offset().left - 100, top: g.text.offset().top - 1 - g.dateeditor.height() });
                } else
                {
                    g.dateeditor.css({ left: g.text.offset().left - 100, top: g.text.offset().top + 1 + g.text.outerHeight() });
                }
            }
            else
            {
