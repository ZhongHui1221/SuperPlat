...
<html>
	...
	<body class="card_body">
					...
					<table>
						<#list page.results as obj>
							<tr <#if obj_index % 2 != 0>style="background-color: #fafafa"</#if>>
								...
								<td style="width: 12%"><a href="javascript: void(0)">${obj["SHENBDH"]}</a></td>
								<td style="width: 11%">${obj["CHUZFS"]}</td>
								<td style="width: 11%"><a href="javascript: void(0)">${obj["PIFDH"]}</a></td>
								<td style="width: 11%">${obj["PIFJE"]}</td>
								<td style="width: 11%">${obj["PIFSL"]}</td>
								<td style="width: 13%">${obj["XUANZSL"]}</td>
								<td style="width: 11%">${obj["XUANZJE"]}</td>
								<td style="width: 12%"><a href="javascript: void(0)">查看流程</a></td>
							</tr>
						</#list>
					</table>
				</div>
		...
		<form action="${request.contextPath}/Query/Czqk/symx" method="post">
			<#-- 额外参数：0 = 处置执行中 -->
			<input name="extra" type="hidden" value="0" />
