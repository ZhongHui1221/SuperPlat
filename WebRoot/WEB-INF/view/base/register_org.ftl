...
<html>
	...
	<body>
		<div class="header">
			新用户注册
			<p>已有账户，马上<a href="${request.contextPath}/${code}">登录</a></p>
		</div>
		<div class="main">
			<form>
				<table class='re-main'>
					<tr>
						<td>真实姓名</td>
						<td><input id="name" name="objname" type="text" /></td>
						<td>*</td>
					</tr>
					<tr>
						<td>手机号</td>
						<td><input id="phone" name="shoujh" type="text" /></td>
						<td>*</td>
					</tr>
					<tr>
						<td>登录账号</td>
						<td><input id="username" name="zhanghm" type="text" /></td>
						<td>*</td>
					</tr>
					<tr>
						<td>密码</td>
					...
					<tr class="po-adjust">
						...
						<td>
							<div class="re-inp">
								<input id="txt1" name="mngdepids" type="text" readonly onclick="showMenu()" />
					...
					<tr>
						...
						<td>
							<p>
								<span class="checked" value="1"></span>是
								<span value="0" style="margin-left: 25px"></span>否
								<input id="sfzb" type="hidden" value="1" />
							</p>
						</td>
					</tr>
					<tr>
						...
						<td>
							<p>
								<span class="checked" value="0"></span>男
								<span value="1" style="margin-left: 25px"></span>女
					...
					<tr>
						<td>邮箱</td>
						<td><input id="email" name="youx" type="text" /></td>
						<td></td>
					</tr>
					<tr>
						<td></td>
			...
			$(function() {
				...
				$(".re-btn").click(function() {
					if (regCanForm()) {
						...
						var email = $("#email").val();
						$.post(
							"${request.contextPath}/CoreUser/register",
							{"orgid": "${orgid}", "objname": name, "shoujh": phone, "zhanghm": username, "psw": psw,
								"mngdepids": depts, "roleids": roles, "zhijid": rank, "shifzb": sfzb, "xingb": gender, "youx": email},
							function(data) {
								if (data == "SUCCESS") {
									location.href = "${request.contextPath}/CoreUser/registered";
								} else {
									alert(data);
								}
							}
